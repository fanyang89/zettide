...
const Transition = cawfs.maintenance.Transition;
...
fn commitTransition(
    ...
    operation_id: cawfs.store.TransactionId,
    mode: cawfs.anchor.Mode,
) !void {
    var transition = try Transition.begin(store, std.testing.allocator, operation_id, mode);
    defer transition.deinit();
    try std.testing.expectEqual(cawfs.maintenance.Outcome.committed, try transition.commit());
    try std.testing.expectEqual(cawfs.maintenance.Status.committed, transition.status());
}
...
test "maintenance coordinator completes a full service-mode cycle" {
    ...
    try std.testing.expectEqual(@as(cawfs.store.TransactionId, @splat(0)), state.control_operation_id);

    var active_bytes = try store.loadImmutable(state.control_ref.?, std.testing.allocator);
    defer active_bytes.deinit();
    const active = try cawfs.maintenance.decode(active_bytes.bytes);
    try std.testing.expectEqual(cawfs.anchor.Mode.maintenance, active.previous_mode);
    try std.testing.expectEqual(cawfs.anchor.Mode.active, active.mode);
    var maintenance_bytes = try store.loadImmutable(active.parent.?, std.testing.allocator);
    defer maintenance_bytes.deinit();
    const maintenance = try cawfs.maintenance.decode(maintenance_bytes.bytes);
    try std.testing.expectEqual(cawfs.anchor.Mode.quiescing, maintenance.previous_mode);
    try std.testing.expectEqual(cawfs.anchor.Mode.maintenance, maintenance.mode);
}
...
test "maintenance coordinator rejects a missing control tip" {
    const missing_ref = cawfs.store.ObjectRef{ .bytes = @splat(0x51) };
...
test "concurrent maintenance operations have one winner" {
    ...
    try std.testing.expectEqual(cawfs.maintenance.Outcome.committed, try first.commit());
    try std.testing.expectEqual(cawfs.maintenance.Outcome.conflict, try second.commit());
    try std.testing.expectEqual(cawfs.maintenance.Status.conflict, second.status());
}
...
test "maintenance resolution distinguishes pending rollback and commit" {
    ...
    try std.testing.expectEqual(cawfs.maintenance.Outcome.indeterminate, try before.commit());
    try std.testing.expectEqual(
        cawfs.maintenance.Resolution.pending,
        try before.resolve(.{}),
    );
    before_model.crash();
    try std.testing.expectEqual(
        cawfs.maintenance.Resolution.not_committed,
        try before.resolve(.{}),
    );
    ...
    try std.testing.expectEqual(cawfs.maintenance.Outcome.indeterminate, try after.commit());
    try std.testing.expectEqual(
        cawfs.maintenance.Resolution.committed,
        try after.resolve(.{}),
    );
    try after.stabilize();
    try std.testing.expectEqual(cawfs.maintenance.Status.committed, after.status());
}
...
test "maintenance resolution finds a transition below a descendant" {
    ...
    try std.testing.expectEqual(cawfs.maintenance.Outcome.indeterminate, try quiescing.commit());
...
test "terminal maintenance resolution handles a rolled-back base" {
    ...
    try std.testing.expectEqual(cawfs.maintenance.Outcome.indeterminate, try maintenance.commit());
    model.crash();
