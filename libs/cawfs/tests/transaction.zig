...
test "transaction publishes and stabilizes an immutable root" {
    ...
    try std.testing.expectEqual(cawfs.transaction.Status.committed, transaction.status());
    ...
    const record = try cawfs.commit.decode(record_bytes.bytes);
    try std.testing.expectEqual(@as(?ObjectRef, null), record.parent);
    try std.testing.expect(cawfs.store.ObjectRef.eql(root, record.root));
}
...
test "transactions publishing the same base have one winner" {
    ...
    try std.testing.expectEqual(cawfs.transaction.Status.committed, first.status());
    try std.testing.expectEqual(cawfs.transaction.Status.conflict, second.status());
}
...
test "transaction resolves an indeterminate applied publication" {
    ...
    try std.testing.expectEqual(cawfs.transaction.Status.indeterminate, transaction.status());
    try std.testing.expectEqual(
    ...
    const current = try cawfs.anchor.decode(&recovered.anchor);
    try std.testing.expectEqual(txn_a, current.transaction_id);
}
...
test "transaction remains pending until another writer wins" {
    ...
    try std.testing.expectEqual(cawfs.resolution.Resolution.pending, try uncertain.resolve(.{}));

    var winner = try Transaction.begin(store, std.testing.allocator, txn_b);
    defer winner.deinit();
    const winner_root = try winner.putImmutable("root-b");
    try std.testing.expectEqual(cawfs.transaction.Outcome.committed, try winner.commit(winner_root));
...
test "transaction retries stabilization without republishing" {
    ...
    defer snapshot.deinit();
    try std.testing.expectEqual(@as(u64, 1), (try cawfs.anchor.decode(&snapshot.anchor)).generation);
}
...
test "transaction resolves a publication rolled back after stabilization failure" {
    ...
    try std.testing.expectEqual(cawfs.transaction.Status.not_committed, transaction.status());
}
...
test "transaction resolves rollback before its unstable base" {
    ...
    try std.testing.expectEqual(cawfs.transaction.Status.not_committed, second.status());
}
...
test "transaction rejects an unavailable root" {
    ...
    try std.testing.expectEqual(cawfs.transaction.Status.staging, transaction.status());
