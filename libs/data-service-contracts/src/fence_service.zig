...
pub const Binding = model.FenceBinding;
pub const Result = model.FenceResult;
...
pub const Store = struct {
    ...
    pub const VTable = struct {
        ...
        find_replica: *const fn (*anyopaque, Id, Id) ?Result,
...
fn findReplica(
    ...
    while (index != 0) {
        ...
        if (std.mem.eql(u8, &binding.volume_id, &volume_id) and
            std.mem.eql(u8, &binding.placement_id, &placement_id)) return records[index];
    }
    return null;
}
...
pub const FileStore = struct {
    ...
    fn decode(allocator: std.mem.Allocator, bytes: []const u8) ![]Result {
        ...
        for (records, 0..) |*record, index| {
            ...
            if (findReplica(records[0..index], record.binding.volume_id, record.binding.placement_id)) |current| {
...
test "epochs advance only after backend drain and reject rollback and conflicts" {
    ...
    var next_generation = testBinding(14, 7);
    next_generation.replica_generation += 1;
    try std.testing.expectError(error.EpochRegression, service.accept(next_generation));
...
test "unknown response is replayed after restart without another drain" {
...

    {
        ...
        try std.testing.expectError(error.StorePoisoned, service.accept(testBinding(11, 6)));
    }
    {
        var file_store = try FileStore.init(std.testing.allocator, std.testing.io, tmp.dir, "fences");
        defer file_store.deinit();
