...
fn allocAlignedForOverwrite(
    allocator: std.mem.Allocator,
    comptime alignment: std.mem.Alignment,
    len: usize,
) ![]align(alignment.toByteUnits()) u8 {
    // Device reads overwrite the entire consumed region before it is inspected.
    const pointer = allocator.rawAlloc(len, alignment, @returnAddress()) orelse
        return error.OutOfMemory;
    return @alignCast(pointer[0..len]);
}
...
pub fn readSnapshotAt(
    ...
    const block_scratch = try allocAlignedForOverwrite(
        allocator,
    ...
    while (consumed < amount) {
        ...
        var entries = uninitialized([blob_device.max_batch]blob_map.LeafEntry);
        ...
        var reads = uninitialized([blob_device.max_batch]blob_store.Store.Read);
        var copies = uninitialized([blob_device.max_batch]struct {
            output_offset: usize,
            block_offset: usize,
            len: usize,
        });
        ...
        if (pending_error == null and entry_index != loaded.count)
            return error.InvalidBlobFileSnapshot;
        if (read_count != 0) {
            var results = uninitialized([blob_device.max_batch]blob_store.Store.ReadResult);
