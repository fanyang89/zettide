...
const blob_data_geometry = @import("../data_mode_geometry.zig").blob;
...
fn validateSet(set: *pool_member_set.PoolMemberSet, writable: bool) !ValidatedSet {
    ...
    for (authority.topology.memberSlice(), 0..) |descriptor, index| {
        ...
        if (header.logical_capacity == 0 or header.data.length < header.logical_capacity)
...
test "scheduled Pool data storage supports heterogeneous members and reordered reopen" {
    ...
    const crossing_offset = blob_data_geometry.chunk_size - io_alignment;
    try storage.writeAllAt(std.testing.io, bytes[0 .. 2 * io_alignment], crossing_offset);
    const writes = [_]storage_api.Write{
        .{ .bytes = bytes[2 * io_alignment .. 3 * io_alignment], .offset = 3 * blob_data_geometry.chunk_size },
        .{ .bytes = bytes[3 * io_alignment ..], .offset = 5 * blob_data_geometry.chunk_size + io_alignment },
    };
...
test "Pool data leaves Blob format geometry to the upper adapter" {
    for ([_]u64{
        blob_data_geometry.chunk_size,
        blob_data_geometry.minimum_device_size + io_alignment,
    }) |logical_capacity| {
        var tmp = std.testing.tmpDir(.{});
        defer tmp.cleanup();
        const member_count = try provisionTestPool(tmp.dir, .unprotected, .blob);
        try rewriteTestLogicalCapacity(tmp.dir, member_count, logical_capacity);
        var set = try openTestSet(tmp.dir, member_count, .writable);

        var storage = try create(std.testing.allocator, std.testing.io, &set, true);
        try std.testing.expectEqual(logical_capacity, storage.capacity());
        try storage.close(std.testing.io);
    }
}
...
fn initOrderedTestContext(context: *Context, replicas: *[max_replica_count]OrderedReplica) !void {
    ...
    for (replicas, &endpoints) |*replica, *endpoint| endpoint.* = .init(replica, .{
        .logical_capacity = blob_data_geometry.minimum_device_size,
        .data_length = blob_data_geometry.minimum_device_size,
    ...
    context.* = .{
        ...
        .device = .{ .mirrored = try .init(
            ...
            &endpoints,
            layout,
            blob_data_geometry.minimum_device_size,
        ) },
        .endpoint_contexts = undefined,
        .data_claims = undefined,
        .data_claim_count = 0,
