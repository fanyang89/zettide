...
> 状态：SPDK 源码已归 `services/data-node/spdk/`，tests/benchmarks 使用显式 roots；独立 data-node executable 的 SPDK composition 待完成
...
1. 全部 SPDK C/Zig wrapper 属于 Linux data-node platform，不进入 `libs/storage-engine/`。
2. `spdk/storage.zig` 是 engine `Storage` port 的 SPDK bdev implementation；依赖方向只能是
   `services/data-node/ SPDK adapter -> zettide_storage`。
...
4. vhost-user-blk、NVMe-oF TCP/RDMA 和 iSCSI 是 block export frontends；它们消费 Catalog Volume，
   不得反向进入 Pool/Catalog。
5. `catalog_endpoint_backend.zig` 是 data-node endpoint composition：它打开 authenticated Pool、创建 export、
   生成 locator，并实现 endpoint registry backend。
6. `zettide-data-node` 最终在单进程内拥有一个 SPDK runtime。CLI compatibility daemon 在迁移完成前可以
   使用同一 composition，但不能与 data-node 同时打开相同 writable Pool 或占用相同 listener/socket。
...
8. 原 Pool production module 对 SPDK 的 test-only 反向 import 已移除；后续 SPDK tests 继续位于 data-node/integration roots。
...
- SPDK adapter 通过 `services/data-node/root.zig` mega-module 访问 engine private files；
...
| 当前文件 | 首轮目标归属 | 职责 |
| --- | --- | --- |
| `spdk/runtime.c/.h/.zig` | `services/data-node` SPDK runtime | SPDK app thread、reactors、runtime lease、start/stop |
| `spdk/nvme_controller.c/.h/.zig` | data-node SPDK ingress | attach/detach remote NVMe controller、namespace bdev names |
| `spdk/bdev_endpoint.c/.h` | data-node SPDK ingress C shim | 在 SPDK thread 打开 bdev、geometry、DMA I/O、remove event |
| `spdk/bdev_dispatcher.c/.h` | data-node SPDK ingress dispatcher | 将 non-SPDK caller dispatch 到 reactor owner，支持 batched reads |
| `spdk/storage.zig` | data-node SPDK Storage adapter | 将 dispatcher 实现为 engine `Storage` port |

### Provider 和 protocol export

| 当前文件 | 首轮目标归属 | 职责 |
| --- | --- | --- |
| `spdk/bdev_provider.c/.h` | data-node SPDK provider shim | 将 external async backend 注册为 SPDK bdev |
| `spdk/provider_bdev.zig` | data-node SPDK provider wrapper | provider create/delete-wait 和 backend callback contract |
| `spdk/vhost_blk_controller.c/.h` | data-node vhost frontend shim | vhost-blk controller create/remove/coalescing |
| `spdk/vhost_block_export.zig` | data-node vhost frontend | provider bdev + controller + socket path owner |
| `spdk/nvmf_tcp_export.c/.h/.zig` | data-node NVMe-oF frontend | subsystem/namespace/listener lifecycle，支持 TCP/RDMA |
| `spdk/iscsi_export.c/.h/.zig` | data-node iSCSI frontend | shared service、portal/initiator group 和 per-target export |

### Catalog bridge 和 endpoint composition

| 当前文件 | 首轮目标归属 | 职责 |
| --- | --- | --- |
| `spdk/catalog_volume_backend.zig` | data-node engine-to-SPDK bridge | lease-bound Catalog Volume worker/provider callback |
| `spdk/catalog_vhost_export.zig` | data-node export composition | Worker + provider bdev + vhost controller |
| `spdk/catalog_nvmf_export.zig` | data-node export composition | Worker + provider bdev + NVMe-oF subsystem |
| `spdk/catalog_iscsi_export.zig` | data-node export composition | Worker + provider bdev + iSCSI target |
| `spdk/catalog_endpoint_backend.zig` | data-node endpoint backend | PoolSource + export lifecycle + locator |

`vendor/spdk/` 继续是 pinned external source dependency。SPDK public headers 和 pkg-config libraries
只出现在 data-node/platform build target，不传播到 `zettide_storage` consumers。
...
- SPDK signal handlers 显式关闭，由 data-node process owner 管理 SIGINT/SIGTERM；
...
目标 data-node lifecycle：

1. 解析并验证 SPDK/data-node config；
2. 启动一个 runtime；
...
任何中间失败都必须反向 unwind 已成功阶段。runtime object、option strings 和 data-node allocator 必须比所有
...
- data-node-owned `PoolMemberSet` 的 `CatalogDataLease`；
...
PoolSource 当前显式验证 location table、重复 location 和 authenticated Pool identity。它属于 data-node
...
3. 将 Worker read/write/flush/reset、out-of-bounds 和 teardown 场景迁移到 data-node SPDK integration test；
4. integration test 只能依赖 public `zettide_storage` facade + data-node SPDK bridge；
...
data-node 内部 C shim interface，不是对用户承诺的独立 SDK。移动时可以与 Zig wrapper 同步调整，但必须：

- 保留 `opts_size` init/validation，避免 C struct version mismatch；
- opaque C handles 不跨 data-node process；
...
`spdk_c` translate-C module 只对 data-node SPDK modules 和专用 tests 可见。`zettide_storage` public root 不导出
SPDK C types。
...
| data-node SPDK runtime/ingress | Linux、runtime/bdev endpoint/dispatcher/NVMe C shims、SPDK libs |
| data-node SPDK provider/exports | runtime、provider/protocol C shims、SPDK event/bdev/NVMf/iSCSI/vhost libs |
| data-node SPDK catalog bridge | `zettide_storage` public Catalog API + provider root |
| data-node endpoint composition | registry + PoolSource + catalog export drivers |
...
这是后续 data-node executable composition 的剩余构建债务。`-Dspdk=true` 继续只允许 Linux；默认
portable build 不探测或链接 SPDK。

当前边界为：

- `zettide_storage` 不导出任何 `spdk_*` symbol；
- `zettide_data_node` 导出 data-node composition 需要的 wrappers；
- benchmark/test roots 显式 import `zettide_data_node` 和 `zettide_storage`；
- legacy `services/data-node/root.zig` 只为兼容 CLI 重导出 SPDK surface；
...
| data-node daemon tests | one runtime owner、reconcile/shutdown、restart、no leaked socket/listener/bdev |
...
- [x] 建立 data-node root 的 SPDK export surface，并从 engine root 排除 `spdk_c`；
- [x] runtime、ingress、provider、protocol export 和 catalog bridge 已归属 `services/data-node/spdk`；
...
- [x] endpoint backend 依赖同 data-node package registry types，不用 mega-module；
- [ ] data-node startup/shutdown 严格执行 runtime owner 顺序；
- [ ] build 不再向 `app_core` 注入完整 SPDK source/library set；
- [x] SPDK Zig tests/benchmarks 改用显式 `zettide_storage` + `zettide_data_node` roots；
