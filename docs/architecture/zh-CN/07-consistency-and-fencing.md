...
Backend-neutral coordinator journal 进一步固定每笔 transaction 的两个 witness：intent 必须先于外部 PREPARE 持久，unknown result 后禁止换第三个 witness；PREPARE acknowledgement 必须通过调用方注入的 authenticated-evidence callback，两个 attestation 持久后才写 certificate，certificate 持久后才允许外部 COMMIT；COMMIT acknowledgement 也必须由独立 callback 校验 pinned witness、exact certificate 和 result，partial progress 可由同 state directory 重启重试。Contracts 不执行网络 fanout 或 authentication，callback 也不能让第三方 participant 独立验证 witness。当前仍没有 confidentiality、在线 key rotation、独立签名 attestation、production 跨节点 primary coordinator 或 recovery quorum。它与当前标准 host-facing NVMf export 无关。
