...
pub fn parse(
    ...
    const fields = type_compat.fields(Self);
            ...
            inline for (fields, 0..) |field, i| {
                ...
                if (!matched_as_flat_oneof) {
                    ...
                    if (@as(
                        ...
                        inline for (type_compat.fields(oneof_type)) |union_field| {
                            if (!matched_as_flat_oneof) {
...
fn stringifyOpts(Self: type, self: *const Self, jws: anytype, opts: Options) std.meta.Child(@TypeOf(jws)).Error!void {
    ...
    inline for (type_compat.fields(Self)) |fieldInfo| {
...
fn stringify_struct_field_with_options(
    ...
    switch (field_descriptor.ftype) {
        ...
        .oneof => |oneof| {
            ...
            inline for (type_compat.fields(@TypeOf(value))) |union_field| {
                if (value == @field(
                    union_info.tag_type.?,
                    union_field.name,
                )) {
...
fn parseEnumField(comptime EnumType: type, allocator: std.mem.Allocator, source: anytype, options: std.json.ParseOptions) !EnumType {
    ...
    inline for (type_compat.enumFields(EnumType)) |field| {
...
fn parseStructField(
    comptime T: type,
    result: *T,
    comptime fieldInfo: type_compat.Field,
    allocator: std.mem.Allocator,
    source: anytype,
    options: std.json.ParseOptions,
) !void {
    @field(result.*, fieldInfo.name) = switch (@field(
        ...
        .oneof => |oneof| oneof: {
            ...
            inline for (type_compat.fields(union_type)) |union_field| {
...
fn fillDefaultStructValues(
    ...
    // Took from std.json source code since it was non-public one
    inline for (type_compat.fields(T), 0..) |field, i| {
        if (!fields_seen[i]) {
