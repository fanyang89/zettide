...
fn countReachableFields(comptime T: type, comptime seen: []const type) usize {
    comptime {
        ...
        var total: usize = 0;
        for (type_compat.fields(@TypeOf(T._desc_table))) |f| {
            total += 1;
...
fn writeValue(
    ...
    switch (comptime field.ftype) {
        ...
        .oneof => |union_type| {
            ...
            inline for (type_compat.fields(@TypeOf(union_type._desc_table))) |union_field| {
...
pub fn encode(
    ...
    inline for (type_compat.fields(Data)) |field| {
...
pub inline fn internal_init(comptime T: type, value: *T) void {
    ...
    @setEvalBranchQuota(comptime evalBranchQuotaFor(T));
    inline for (type_compat.fields(T)) |field| {
    ...
    switch (ftype) {
        ...
        .oneof => |one_of| {
            ...
            if (@field(original, field_name)) |union_value| {
                ...
                inline for (type_compat.fields(@TypeOf(one_of._desc_table))) |union_field| {
