...
pub fn StreamDecoder(comptime T: type) type {
    return struct {
        ...
        fn dispatch(self: *Self, tag: wire.Tag) Error!?Event {
            ...
            inline for (type_compat.fields(@TypeOf(desc_table))) |sf| {
                ...
                if (comptime field_desc.ftype == .oneof) {
                    ...
                    inline for (type_compat.fields(@TypeOf(inner))) |oo| {
...
fn EventUnion(comptime T: type) type {
    ...
    for (type_compat.fields(@TypeOf(desc_table))) |sf| {
        ...
        if (field_desc.ftype == .oneof) {
            ...
            for (type_compat.fields(@TypeOf(inner))) |oo| {
