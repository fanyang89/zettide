...
fn generateFormatTo(comptime Args: type, comptime format: []const u8) FormatToFn {
    const Closure = struct {
        fn formatTo(data: []const u8, writer: *std.Io.Writer) !void {
            ...
            inline for (struct_info.field_names, struct_info.field_types, struct_info.field_attrs) |field_name, T, attrs| {
                ...
                switch (info) {
                    ...
                    .pointer => |p| {
                        if (p.size == .slice and p.child == u8) {
                            ...
                            @field(args, field_name).ptr = @ptrCast(@constCast(data[idx..].ptr));
                            idx += len;
