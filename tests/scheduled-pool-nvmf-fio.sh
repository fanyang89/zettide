...
benchmark_driver=${ZETTIDE_SCHEDULED_POOL_BENCHMARK_DRIVER:-tests/spdk-nvmf-fio.sh}
...
if [[ $storage_transport == synthetic ]]; then
    [[ $benchmark_mode == pool && $raw_windows == 0 && -z $expected_pool_id &&
        $benchmark_driver == tests/spdk-vhost-user-blk-fio.sh ]] || {
    ...
    if [[ $benchmark_driver != tests/* || $benchmark_driver == *..* || ! -x $benchmark_driver ]]; then
        echo "benchmark driver must be an executable repository-relative tests/ path without .." >&2
        exit 2
    fi
...
if [[ $benchmark_mode == raw_nvme ]] &&
    [[ $storage_transport != spdk_nvme_pcie || $pcie_preparation_mode != validate ||
        ! $expected_pool_id =~ ^[0-9a-f]{32}$ || $benchmark_driver != tests/spdk-vhost-user-blk-fio.sh ]]; then
