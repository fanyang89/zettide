...
"${CC:-cc}" -std=c11 -D_GNU_SOURCE -Wall -Wextra -Werror -Wno-unused-parameter -Iservices/data-node \
	services/data-node/spdk/runtime.c services/data-node/spdk/bdev_endpoint.c services/data-node/spdk/bdev_dispatcher.c tests/spdk_dispatcher.c \
...
LD_LIBRARY_PATH="${library_path}${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
	timeout 30 "$build_dir/zettide-spdk-dispatcher-test"
