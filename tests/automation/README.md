...
Run all commands in this document from the automation project directory:
...
```sh
uv run ansible-playbook io-verify.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook blob-fuse-fio.yml --limit zettide-pm883
```
...
```sh
uv run ansible-playbook raw-device.yml --limit zettide-tier1 \
...
```sh
uv run ansible-playbook pool-fio.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook blob-pool-fio.yml --limit zettide-tier1 \
...
```sh
uv run ansible-playbook blob-pool-nfs-fio.yml --limit zettide-tier1 \
  -e 'zettide_blob_pool_fio_confirm=DESTROY:/dev/nvme1n1:PHYSICAL-DISK-SERIAL'
```
...
```sh
uv run ansible-playbook throughput.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook nvmf-fio.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook nvmf-catalog-rxe-fio.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook nvmf-scheduled-pool-rxe-fio.yml --limit zettide-tier1 \
...
```sh
uv run ansible-playbook vhost-scheduled-pool-fio.yml \
  --limit zettide-tier1 \
...
```sh
uv run ansible-playbook spdk-nvme-perf.yml \
  --limit zettide-tier1 \
  -e '{"zettide_raw_device":{"path":"/dev/nvme1n1","serial":"PHMB747600DE280CGN","secondary":{"path":"/dev/nvme0n1","serial":"PHMB746600SS280CGN"}}}' \
  -e zettide_vhost_scheduled_pool_storage_transport=spdk_nvme_pcie \
...
```sh
uv run ansible-playbook vhost-spdk-nvme-fio.yml \
  --limit zettide-tier1 \
  -e '{"zettide_raw_device":{"path":"/dev/nvme1n1","serial":"PHMB747600DE280CGN","secondary":{"path":"/dev/nvme0n1","serial":"PHMB746600SS280CGN"}}}' \
  -e zettide_vhost_scheduled_pool_storage_transport=spdk_nvme_pcie \
  -e zettide_vhost_scheduled_pool_pcie_preparation_mode=validate \
...
```sh
uv run ansible-playbook nvmf-catalog-fio.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook nvmf-catalog-mapped-fio.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook nvmf-catalog-optane-fio.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook blob-device.yml --limit zettide-tier1
```
...
```sh
uv run ansible-playbook blob-workload.yml --limit zettide-tier1
```
