...
const filesystem_target = @import("filesystem_target.zig");
const linux_block_device = @import("v3/linux_block_device.zig");
const nfs_handle = @import("nfs_handle.zig");

const allocator = std.heap.c_allocator;
const filesystem_api = storage_engine.nfs_filesystem;
...
const BlobOwner = struct {
    native: storage_engine.blob_filesystem.Filesystem,
    adapter: storage_engine.nfs_blob_adapter.Adapter,
};
...
const FilesystemOwner = struct {
    ...
    fn openInto(
        ...
        switch (try filesystem_target.classifyPath(io, path)) {
            .blob => {
                self.* = .{ .blob = undefined };
                self.blob.native = try filesystem_target.openBlobFilesystem(
                    allocator_value,
                    io,
                    path,
                    writable,
    ...
    fn openPoolInto(
        ...
        switch (try set.dataMode()) {
            ...
            .blob => {
                self.* = .{ .blob = undefined };
                self.blob.native = try filesystem_target.openBlobPoolFilesystem(
                    allocator_value,
                    io,
                    &set,
                    writable,
...
fn openSingleMemberPool(
    ...
    path: []const u8,
    writable: bool,
) !storage_engine.v3.pool_member_set.PoolMemberSet {
    const target_stat = try std.Io.Dir.cwd().statFile(io, path, .{});
    const storage = switch (target_stat.kind) {
        .file => try storage_engine.v3.storage.Storage.openFile(io, std.Io.Dir.cwd(), path, writable),
        .block_device => if (@import("builtin").os.tag == .linux)
            (try linux_block_device.openStorageOptions(
                io,
                allocator_value,
                path,
                writable,
    ...
    var storages = [_]storage_engine.v3.storage.Storage{storage};
    return storage_engine.v3.pool_member_set.PoolMemberSet.openStorages(
        io,
        allocator_value,
        &storages,
...
test "direct NFS backend resolves and reads stable handles" {
    ...
    try filesystem_target.formatNewBlobFile(
        std.testing.io,
    ...
    {
        var filesystem = try filesystem_target.openBlobFilesystem(
            std.testing.allocator,
            std.testing.io,
            path,
            true,
    ...
    try std.testing.expectEqual(@intFromEnum(storage_engine.metadata.Kind.directory), root_attributes.kind);
...
test "direct NFS backend exports a single-member Blob Pool" {
    ...
    var storages = [_]storage_engine.v3.storage.Storage{
        try storage_engine.v3.storage.Storage.createFile(std.testing.io, tmp.dir, member_name, 16 * 1024 * 1024),
    };
    ...
    var native = try filesystem_target.formatProvisionedBlobPool(
        std.testing.allocator,
        std.testing.io,
        &provisioned,
        .portable_v1,
