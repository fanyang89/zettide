...
| Distributed data plane | Foundation | Three-node file-backed control lifecycle reaches `ACTIVE` and restart failover advances the write epoch; a focused local participant proves durable two-witness PREPARE/COMMIT ordering, while cross-node user-write transport, repair, and managed publication remain future work |
